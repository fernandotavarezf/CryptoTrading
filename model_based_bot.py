...
def calc_size(equity: float, price: float, ticker: str,
    ...
    """Return order quantity given equity and stop loss."""
    return max(
        (equity * risk_pct) / (price * stop_loss_pct) * leverage,
        OrderManager.get_min_size(ticker),
    )
...
class MLTradingBot:
    ...
    async def _open_long_position(self, ticker: str, signal: StrategySignal, equity: float) -> None:
        ...
        order = OrderRequest(symbol=ticker, side="buy", order_type=signal.order_type, quantity=position_size, price=signal.entry_price)
    ...
    async def _close_position(self, ticker: str, info: Dict, reason: str) -> None:
        ...
        order = OrderRequest(symbol=ticker, side=close_side, order_type="market", quantity=info["position_size"], reduce_only=True)
        await self.exchange.create_order(order)
